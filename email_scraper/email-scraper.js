...
import { navigateTo, waitForCloudflareChallenge, navigateToRelated } from "./pageNavigator.js";
import { saveToFile, fallbackScraping, getJsonContent } from "./dataExtractor.js";
...
export async function scrapeEmailList() {
    ...
    try {
        ...
        const homepageResponse = await navigateTo(page, homepageUrl, "domcontentloaded", navigationTimeout);
        if (!homepageResponse || homepageResponse.status() !== 200) {
            console.error(`❌ Failed to load homepage: ${homepageResponse ? homepageResponse.status() : 'No response'}`);
        ...
        try {
            ...
            const apiResponse = await navigateTo(page, apiUrl, "domcontentloaded", navigationTimeout); // Await the navigation

            if (apiResponse && apiResponse.status() === 200) { // Check status() method
                ...
                if (jsonContent && Array.isArray(jsonContent)) {
                    ...
                    console.log("❌ Could not parse JSON data from API response or data is not an array.");
                }
                ...
                console.log("🔄 Attempting to scrape broker data from website pages as a fallback...");
                await fallbackScraping(page);
            }

        } catch (apiError) {
            console.log("❌ Could not access API endpoint directly:", apiError.message);
            // Fallback: Try to find broker data on regular pages
            console.log("🔄 Attempting to scrape broker data from website pages as a fallback...");
            await fallbackScraping(page);
        }
